...
//  Elements: General purpose conversion of state vector to orbital elements at specified epoch:
//  elliptic, parabolic, hyperbolic (this function added by J. Vilagi)
//
// Input:
//
//   GM       Product of gravitational constant and centre mass [AU^3*d^-2]
...
//   Omega    Longitude of the ascending node of the orbit in [rad]
//   omega    Argument of perihelion in [rad]
//   Epoch    epoch of r, v and the elements...[d]
//   T0       time od perihelion passage [d]
//   a        Semimajor axis of the orbit in [AU]
//   M        Mean anomaly in [rad]
//
//------------------------------------------------------------------------------
void Elements ( double GM, double Epoch, const Vec3D& r, const Vec3D& v,
                double& q, double& e, double& i,
                double& Omega, double& omega, double& T0, double& a,double& M ) {
   //
   // Variables
   //
   Vec3D  h;
   ...
   if (fabs(denom)<eps_mach) {                        //parabolic case
      e=1;
      a=numeric_limits<double>::infinity();
      q=H*H/2/GM;
      tan_nu = sgn(Dot(r,v))*sqrt(R/q-1);
      nu = 2*atan(tan_nu);
      M=tan_nu*(1+tan_nu*tan_nu/3);
      T0=Epoch-M*sqrt(2*q*q*q/GM);
   } else {
      a = 1.0/denom;                                   // Semi-major axis
      eCosE = 1.0-R/a;                                 // e*cos(E) ... or e*cosh(H) for hyp.
      eSinE = Dot(r, v)/sqrt(GM*fabs(a));              // e*sin(E) ... or e.sinh(H) for hyp.
      e2 = eCosE*eCosE+sgn(a)*eSinE*eSinE;
      e  = sqrt(e2);                                   // Eccentricity
      if (a>0) {
         E  = atan2(eSinE,eCosE);                       // Eccentric anomaly
         M  = E-eSinE;                                  // Mean anomaly
         ...
         harg=eSinE/eCosE;
         E  = log((1+harg)/(1-harg))/2;                 // Eccentric anomaly (hyp.)
         M  = eSinE-E;                                  // Mean anomaly (hyp.)
         nu = atan2(sqrt(e2-1)*eSinE, e2-eCosE);        // True anomaly (hyp.)
      }
      q=a*(1-e);
      T0=Epoch-M*sqrt(fabs(a*a*a)/GM);
   }
