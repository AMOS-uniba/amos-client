...
void ETminUT (double T, double& DTsec, bool& valid) {
    ...
    if ( (T<-1.75) || (0.05<T) ) {
        ...
        switch (i) {
            case -7:
            DTsec=10.4+t*(-80.8+t*( 413.9+t*( -572.3)));
            break; // 1825-
            case -6:
            DTsec= 6.6+t*( 46.3+t*(-358.4+t*(   18.8)));
            break; // 1850-
            case -5:
            DTsec=-3.9+t*(-10.8+t*(-166.2+t*(  867.4)));
            break; // 1875-
            case -4:
            DTsec=-2.6+t*(114.1+t*( 327.5+t*(-1467.4)));
            break; // 1900-
            case -3:
            DTsec=24.2+t*( -6.3+t*(  -8.2+t*(  483.4)));
            break; // 1925-
            case -2:
            DTsec=29.3+t*( 32.5+t*(  -3.8+t*(  550.7)));
            break; // 1950-
            case -1:
            DTsec=45.3+t*(130.5+t*(-570.5+t*( 1516.7)));
            break; // 1975-
            case  0:
            t+=0.25;
...
//
//------------------------------------------------------------------------------
void CalDat ( double Mjd, int& Year, int& Month, int& Day, double & Hour ) {
    //
    // Variables
    //
    long    a,b,c,d,e,f;
...
//
//------------------------------------------------------------------------------
void CalDat ( double Mjd, int& Year, int& Month, int& Day, int& Hour, int& Min, double& Sec ) {
    //
    // Variables
    //
    double Hours;
...
ostream& operator << (ostream& os, const Time& time) {
    ...
    switch ( time.m_Format ) {
        ...
        os.fill('0');
        os << setw(2) << H << "h" << setw(2) << M << "m" << setw(2) << int(S) << "s";
        os.fill(' ');
        ...
        Hour = floor(fac*fabs(time.m_Hour)+0.5)/fac+0.1/fac;
        DMS (Hour, H, M, S);
        os.fill('0');
        os << setw(2) << H << "h" << setw(2) << M << "m" << fixed  << setw(3+p) << S << "s";
        os.fill(' ');

        break;

        default:
            os << "wrong TimeFormat for Time object..." <<  endl;
