...
//
//------------------------------------------------------------------------------
Vec3D Site (double lambda, double phi, double alt) {
    //
    // Constants
    //
    ...
    const double cos_lam = cos(lambda);    // (Co)sine of geographical longitude
    const double sin_lam = sin(lambda);
    ...
    return Vec3D ( N*cos_phi*cos(lambda)+alt*cos_phi*cos_lam,
                   N*cos_phi*sin(lambda)+alt*cos_phi*sin_lam,
                   (1.0-e_sqr)*N*sin_phi+alt*sin_phi  );
}
