...
namespace // Unnamed namespace
{
    ...
    void ILE_Pert::Init (double T) {
        ...
        for (int i=0; i<=3; i++) {
            ...
            Cos[o][i]=1.0;
            Cos[o+1][i]=cos(arg)*fac;
            Cos[o-1][i]=+Cos[o+1][i];
            Sin[o][i]=0.0;
            Sin[o+1][i]=sin(arg)*fac;
            Sin[o-1][i]=-Sin[o+1][i];
    ...
    void ILE_Pert::Term (int p, int q, int r, int s, double& x, double& y) {
        ...
        for (int k=0; k<=3; k++)
            if (i[k]!=0)
                AddThe(x,y,Cos[o+i[k]][k],Sin[o+i[k]][k],x,y);
    }
...
Vec3D MoonPos (double T) {
    ...
    Pert.AddN(-526.069, 0, 0,1,-2);
    Pert.AddN(  -3.352, 0, 0,1,-4);
    Pert.AddN( +44.297,+1, 0,1,-2);
    Pert.AddN(  -6.000,+1, 0,1,-4);
    Pert.AddN( +20.599,-1, 0,1, 0);
    Pert.AddN( -30.598,-1, 0,1,-2);
    Pert.AddN( -24.649,-2, 0,1, 0);
    Pert.AddN(  -2.000,-2, 0,1,-2);
    Pert.AddN( -22.571, 0,+1,1,-2);
    Pert.AddN( +10.985, 0,-1,1,-2);
