...
ostream& operator << (ostream& os, const Angle& alpha) {
    ...
    double S;         // seconds
    ios_base::fmtflags flags;     // output stream format flags
    int    sp;    // width, precision, showpos
    ...
    switch (alpha.m_Format) {
        ...
        DegRound = floor(60.0*fabs(alpha.m_angle)+0.5)
                   /60.0+0.00001;
        ...
        if (alpha.m_angle<0.0)
            ...
            os << setw(w-7-p);
        os << D << ":"  << setfill('0') << setw(2) << M << ":" << fixed << setw(3+p)  << S << setfill(' ');
